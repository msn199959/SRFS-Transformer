...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        memory, intermediate_memory = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed) #torch.Size([64, batch_size/img_patches, 256])
        hs, references = self.decoder(tgt, memory, memory_key_padding_mask=mask,
                                      pos=pos_embed, query_pos=query_embed)

        return hs, references, intermediate_memory
...
class TransformerEncoder(nn.Module):
    ...
    def visualize_and_save_feature_maps_as_images(self, feature_maps, img_id, layer_num):
        """
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        output = tgt  #[num_queries, batch_size, 256]

        intermediate = []

        reference_points_before_sigmoid = self.ref_point_head(query_pos)  # [num_queries, batch_size, 2]
        reference_points = reference_points_before_sigmoid.sigmoid().transpose(0, 1) #[batch_size, num_queries, 2]
        ...
        #保存decoder layer的中间结果，用于对中间结果进行额外的监督训练
        if self.return_intermediate:
            return [torch.stack(intermediate).transpose(1, 2), reference_points]

        return output.unsqueeze(0) #[1,num_queries, H, W]
